        ...
        <Grid size={{ xs: 12, sm: 6, lg: 4 }} key={p.id}>
          ...
          >
            <CardContent sx={{ flex: 1, pb: 1 }}>
              ...
              <Typography variant="body1" sx={{ fontWeight: 500, lineHeight: 1.3, mb: 0.5 }}>
                {isAr && p.name_ar ? p.name_ar : p.name_en}
              </Typography>
              {((isAr && p.name_en) || (!isAr && p.name_ar)) && (
                <Typography variant="body2" sx={{ color: "text.disabled", fontSize: 12, mb: 1 }}>
                  {isAr ? p.name_en : p.name_ar}
                </Typography>
              )}
            ...
            <CardActions sx={{ px: 1.5, pt: 0, justifyContent: "flex-end" }}>
              ...
              <Tooltip title={deleteConfirm === p.id ? t("common.confirm") : t("common.delete")}>
                <IconButton
                  size="small"
                  onClick={() => handleDelete(p.id)}
                  sx={{ color: deleteConfirm === p.id ? "error.main" : "inherit" }}
                >
                  <DeleteIcon sx={{ fontSize: 16 }} />
                </IconButton>
              </Tooltip>
